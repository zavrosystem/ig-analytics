  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center px-6">
      <div className="flex flex-col items-center text-center max-w-lg">
        ...
        <h1 className="text-6xl font-black text-gray-900 mb-6 uppercase tracking-tight whitespace-nowrap">
          ¡Lamentamos las molestias!
        </h1>
        <p className="text-2xl text-gray-500 mb-3 whitespace-nowrap">{msg}</p>
        <p className="text-xl text-gray-400 whitespace-nowrap">Pronto estará disponible nuevamente.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex h-screen bg-[#F6F5F3] overflow-hidden">
      ...
      <aside className="w-[210px] h-screen bg-white border-r border-gray-100 flex flex-col shrink-0">
        ...
        {/* Ajustes + Config — just above the divider */}
        <div className="px-3 pb-2">
          <NavItem
            icon={<Settings className="w-full h-full" />}
            label={isAdmin ? "Config" : "Ajustes"}
        ...
        <div className="px-6 py-6 pb-10 space-y-5">
          ...
          {activeNav === "reels" && (
            <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-6">
              <ContentSection clientId={selectedClientId} clientName={clientName} />
            </div>
          )}
          ...
          {activeNav === "config" && (
            <div className="space-y-5">
              <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-8 max-w-lg">
                <h2 className="text-base font-bold text-gray-900 mb-1">Ajustes de cuenta</h2>
                <p className="text-xs text-gray-400 mb-6">Conecta o reconecta tu cuenta de Instagram.</p>
                <a
                  href={`https://zavrosystem--ig-oauth-login.modal.run?user_id=${session.user.id}&client_id=${selectedClientId ?? ""}&name=${encodeURIComponent(clientName)}`}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#FF7200] text-white text-sm font-semibold hover:bg-[#e56600] transition-colors"
                >
